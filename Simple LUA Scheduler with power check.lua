...
pluggID = {84};			-- ID of device controlled
control = "styringVVB"	-- Name of your CONTROL variable
status = "statusVVB"	-- Name of your STATUS variable
timeoff = {6,12,15,18};	-- Time the device is turned off between. Here it is between 6 and 12 and between 15 and 18. 
timeoffnr = 4			-- Insert how many times you have in timeoff
timeon = {18,20,3,6};	-- Time the device is turned on between. Here it is between 18 and 20 and between 3 and 6. 
timeonnr = 4			-- Insert how many times you have in timeon
powerlimit = 10			-- How many watts used to set device in Running status
...
-- 1) Change number 84 to your device ID
-- 2) Change styringVVB to your control variable name
-- 3) Change statusVVB  to your status variable name
-----------------------------------------------------------
...
-- checkStatus, functions
checkStatus = function()
  for i = 1,#pluggID do
    pluggID2 = pluggID[i];
    local power = fibaro:getValue(pluggID2, "power");
    local status2 = fibaro:getValue(pluggID2, "value");
    if (tonumber(power) > tonumber(powerlimit)) then
      if (status2 == "1" and status ~= "Running") then
        fibaro:setGlobal(status, "Running");
      if debug == true then Debug( "green", "Device is running") end;
      end
    end
    if (tonumber(power) < tonumber(powerlimit)) then
      if (status2 == "1" and status ~= "On") then
        fibaro:setGlobal(status, "On");
      end
    end
    if (tonumber(power) < tonumber(powerlimit)) then
      if (status2 == "0" and status ~= "Off") then
        fibaro:setGlobal(status, "Off");
      end
    end
  end
...
credits = function()
  version = "1.0"
  Debug( "orange", "Simple LUA Scheduler with power check - LUA Scripting by Tor Gaute Lien - 2018" );
